...
class DDA:
    ...
    def calculate_rotation_matrix(self, theta_x, theta_y, theta_z):
        ...
        rot_x = tf.constant([[1.0, 0.0, 0.0],
                             ...
                             [0.0, np.sin(theta_x),  np.cos(theta_x)]], dtype=tf.complex128)

        rot_y = tf.constant([[ np.cos(theta_y), 0.0, np.sin(theta_y)],
                             [ 0.0, 1.0, 0.0],
                             [-np.sin(theta_y), 0.0, np.cos(theta_y)]], dtype=tf.complex128)

        rot_z = tf.constant([[np.cos(theta_z), -np.sin(theta_z), 0.0],
                             [np.sin(theta_z),  np.cos(theta_z), 0.0],
                             [0.0, 0.0, 1.0]], dtype=tf.complex128)
    ...
    def calculate_heter_polarizability(self):
        ...
        ref_target_pure = np.array([(self.n_real[i](self.wl_exp) + 1j*self.n_imag[i](self.wl_exp)).astype("complex128") for i in range(len(self.n_real))])
        ref_targets = []
        ...
        self.alpha_j = np.array(alpha_j_total).astype("complex128")
        return ref_targets
    ...
    def calculate_Amatrix(self, r, k, alpha_j):
        ...
        rij = tf.reshape(tf.norm(rS,axis=(2,3)) + tf.eye(len(r), dtype=tf.complex128), (len(r), len(r), 1, 1))
        rS_norm = rS/rij
        rS2 = tf.matmul(tf.linalg.matrix_transpose(rS_norm), rS_norm)
        
        norm_k = tf.cast(tf.norm(k),tf.complex128)

        rij = tf.cast(rij,tf.complex128)
        rS2 = tf.cast(rS2,tf.complex128)

        # Calculate A matrix from the Green's function formulation
        A_matrix = tf.exp(1j*norm_k*rij)/rij*(
            ((norm_k)**2)*(rS2-tf.eye(3, dtype=tf.complex128)) + 
            (1j*norm_k*rij-1)/(rij**2)*(3*rS2 - tf.eye(3, dtype=tf.complex128)))
        ...
        A_matrix = tf.linalg.set_diag(A_matrix, tf.zeros(A_matrix.shape[0], dtype=tf.complex128), name=None)+tf.cast(tf.linalg.tensor_diag(np.repeat(1/alpha_j,3)),dtype = tf.complex128)
#         + 1/alpha_j*tf.eye(A_matrix.shape[0], dtype=tf.complex128)
        return A_matrix
    ...
    def run_DDA(self):
        ...
        """
        # self.r_old = self.dl*tf.constant(self.dipole_list, dtype=tf.complex128)
        self.r_old = tf.constant(self.dipole_list, dtype=tf.complex128)

        if self.calculate_electricfield:
            self.mesh_filepath = str(self.folder_path) + '/mesh_coordinates.csv'
            self.read_mesh_data()
            self.mesh_list = tf.constant(self.mesh_list, dtype = tf.complex128)
        ...
        for index_wave in range(len(self.wl_exp)):
            ...
            with tf.device(self.gpu_device):
                ...
                E_in0 = tf.constant([[1,0,0]], dtype=tf.complex128)
                E_pol = tf.constant([[0],[0],[-self.k[index_wave]]], dtype=tf.complex128)
    ...
    def calculate_local_electrical_field(self, r_position, index_wave, dipoles, E_pol, E_in0):
        ...
        with tf.device(self.gpu_device):
            ...
            norm_k=tf.cast(tf.norm(self.k[index_wave]),tf.complex128)

            # calculate the A_matrix
            A_matrix=tf.exp(1j*norm_k*r_norm)/r_norm*(
                (norm_k**2)*(r_3by3-tf.eye(3,dtype=tf.complex128))+
                (1j*norm_k*r_norm-1)/(r_norm**2)*(3*r_3by3-tf.eye(3,dtype=tf.complex128)))
    ...
    def calculate_spectrum_trajectories(self,np_dda1,np_dda2,index_wave,k):
        r_old = tf.constant(np_dda1.dipole_list,dtype=tf.complex128)
        r_new = tf.constant(np_dda2.dipole_list,dtype=tf.complex128)
        with tf.device(self.gpu_device):
            ...
            #set the initial electric field
            E_in0=tf.constant([[1,0,0]],dtype=tf.complex128)
            E_pol=tf.constant([[0],[0],[-k[index_wave]]],dtype=tf.complex128)
            ...
            E_j_new=tf.cast(E_j_new,tf.complex128)
            ...
            for atom_index in range(A_matrix1_shape,A_matrix2.shape[0]):
                ...
                C_extra2=-np_dda2.alpha_j[int(atom_index/3),index_wave]*tf.transpose(tf.concat([C,-tf.ones((1,1),dtype=tf.complex128)],axis=0))
...
#                     print(est)
            ...
            self.estimations.append(estimations)
        ...
        for index_wave in range(len(self.wl_exp)):
            ...
            with tf.device(self.gpu_device):
                ...
                for atom_index in self.flip_infor[0]:    
                    ...
                    prefactor=(1/alpha_j2[index_wave]-1/alpha_j1[index_wave])/(1+(1/alpha_j2[index_wave]-1/alpha_j1[index_wave])*A_matrix_reverse[atom_index,atom_index])
#                     print(A_matrix_reverse[atom_index,atom_index])
#                     print(alpha_j1[index_wave])
                    dipoles=dipoles-prefactor*A_matrix_reverse[:,atom_index:atom_index+1]*dipoles[:,atom_index:atom_index+1,:]
                    A_matrix_reverse=A_matrix_reverse-prefactor*tf.linalg.matmul(A_matrix_reverse[:,atom_index:atom_index+1],A_matrix_reverse[atom_index:atom_index+1,:])
    ...
    def calculate_spectrum_trajectories_v4(self,scale_factor = 10**(-5),tmp_paths = [],wavelength_index =1,max_step = 1000):
        """
        Runs Discrete Dipole Simulation to calculate the UV-Vis Spectra of the given nanoparticle
        """
        self.r_old = tf.constant(self.dipole_list, dtype=tf.complex128)
        # Set the expeirmental path
        data_path = tmp_paths[0]
        C_cross_file_path = tmp_paths[1]
        flag_tmp_path = tmp_paths[2]
        sampling_tmp_path =  tmp_paths[3]
        if self.calculate_electricfield:
            self.mesh_filepath = str(self.folder_path) + '/mesh_coordinates.csv'
            self.read_mesh_data()
            self.mesh_list = tf.constant(self.mesh_list, dtype = tf.complex128)
        ...
        for index_wave in range(len(self.wl_exp)):
            ...
            with tf.device(self.gpu_device):
                ...
                dipoles = tf.matmul(A_matrix_reverse, E_j)
                C_cross_temp = 4*np.pi*self.k[index_wave]*tf.reduce_sum(tf.math.imag(tf.math.conj(E_j)*dipoles))/E_j.shape[0]*scale_factor
                # Save the C_cross for the first initial structure
                np.savetxt(C_cross_file_path+f'/C_cross_{step}_{wavelength_index}.csv',np.array([C_cross_temp]),delimiter=',')
                for step in range(1,max_step):
                    print(step)
                    sample_path = sampling_tmp_path+f'/{step}_{wavelength_index}.csv'
                    print(sample_path)
                    while True:
                        flag1 = False
                        flag2 = False
                        flag1 = os.path.isfile(sample_path)
                        try:
                            os.rename(sample_path,sample_path)
                            time.sleep(0.1)
                            flag2 = True
                        except:
                            pass
                        if flag1 and flag2:
                            dipole_index = int(np.loadtxt(sample_path,delimiter=',').item())
                            #changing the dielectric property of individual dipoles
                            for atom_index in range(3*dipole_index,3*dipole_index+3):    
                                alpha_j1 = self.flip_infor[1][atom_index]/scale_factor
                                alpha_j2 = self.flip_infor[2][atom_index]/scale_factor
                                ...
                                dipoles=dipoles-prefactor*A_matrix_reverse[:,atom_index:atom_index+1]*dipoles[:,atom_index:atom_index+1,:]
                                A_matrix_reverse=A_matrix_reverse-prefactor*tf.linalg.matmul(A_matrix_reverse[:,atom_index:atom_index+1],A_matrix_reverse[atom_index:atom_index+1,:])
                                # Swap the polarizability after everything is done
                                self.flip_infor[1][atom_index],self.flip_infor[2][atom_index] = self.flip_infor[2][atom_index],self.flip_infor[1][atom_index]
                            # Store the C_cross 
                            C_cross_temp=4*np.pi*self.k[index_wave]*tf.reduce_sum(tf.math.imag(tf.math.conj(E_j)*dipoles))/E_j.shape[0]*scale_factor
                            np.savetxt(C_cross_file_path+f'/C_cross_{step}_{wavelength_index}.csv',np.array([C_cross_temp]),delimiter=',')
                            # Check if it passes the simulated annealing
                            flag_path = flag_tmp_path+f'/flag_{step}_{wavelength_index}.csv'
                            while True:
                                flag1 = False
                                flag2 = False
                                flag1 = os.path.isfile(flag_path)
                                try:
                                    os.rename(flag_path,flag_path)
                                    time.sleep(0.1)
                                    flag2 = True
                                except:
                                    pass
                                if flag1 and flag2:
                                    ...
                                    change_flag = int(np.loadtxt(flag_path,delimiter=',').item())
                                    if change_flag == 1:
                                        pass
                                    else:
                                        # The algorithm refused the change, so we need to change the structure back
                                        for atom_index in range(3*dipole_index,3*dipole_index+3):    
                                            alpha_j1 = self.flip_infor[1][atom_index]/scale_factor
                                            alpha_j2 = self.flip_infor[2][atom_index]/scale_factor
    ...
    def calculate_spectrum_trajectories_v5(self,scale_factor = 10**(-5),tmp_paths = [],wavelength_index =1,max_step = 1000):
        """
        Runs Discrete Dipole Simulation to calculate the UV-Vis Spectra of the given nanoparticle
        """
        self.r_old = tf.constant(self.dipole_list, dtype=tf.complex128)
                ...
                for step in range(1,max_step):
                    ...
                    while True:
                        ...
                        try:
                            ...
                            pass
                        if flag1 and flag2:
                            dipole_index_total = [int(i) for i in np.loadtxt(sample_path,delimiter=',')]
                            for dipole_index in dipole_index_total:
                                #changing the dielectric property of individual dipoles
                                for atom_index in range(3*dipole_index,3*dipole_index+3):    
                                    alpha_j1 = self.flip_infor[1][atom_index]/scale_factor
                                    alpha_j2 = self.flip_infor[2][atom_index]/scale_factor
    ...
    def calculate_spectrum_trajectories_v2_1(self):
        """
        Runs Discrete Dipole Simulation to calculate the UV-Vis Spectra of the given nanoparticle
        """
        # self.r_old = self.dl*tf.constant(self.dipole_list, dtype=tf.complex128)
        ...
        for index_wave in range(len(self.wl_exp)):
            ...
            with tf.device(self.gpu_device):
                ...
                for atom_index in self.flip_infor[0]:    
                    ...
                    # Flip the informatoin after flipping the dipoles, so that in the future when we need to flip it back, we can. 
                    # It should be noted we only swap alpha in the current wavelength
                    self.flip_infor[1][atom_index][index_wave], self.flip_infor[2][atom_index][index_wave] = self.flip_infor[2][atom_index][index_wave], self.flip_infor[1][atom_index][index_wave]
    ...
    def calculate_spectrum_trajectories_v2_2(self):
        """
        Runs Discrete Dipole Simulation to calculate the UV-Vis Spectra of the given nanoparticle
        """
        # self.r_old = self.dl*tf.constant(self.dipole_list, dtype=tf.complex128)
        ...
        for index_wave in range(len(self.wl_exp)):
            ...
            with tf.device(self.gpu_device):
                ...
                #changing the dielectric property of individual dipoles
                for atom_index in self.flip_infor[0]:    
                    alpha_j1 = self.flip_infor[1+int(atom_index_count/3)][atom_index]
                    alpha_j2 = self.flip_infor[2+int(atom_index_count/3)][atom_index]
