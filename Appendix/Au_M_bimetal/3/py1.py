...
#define the kinetic constant for simulation example_-4.0_-4.0_1.0_1.0_0.4_1.0_500000
k_Au=-4.0
k_Ag=-4.0
Energy_ratio=1.0
Energy_ratio2=1.0
...
def addition_event(flag=False):
    ...
    return flag,site_surface_site_temp,atom_type
...
def replacement_event(flag=False):
    ...
    return flag,site_temp,atom_type
...
max_coor_num=12
position=np.loadtxt("/home/yibin/workspace/Growth_with_silverV3_traj/atomList_TruncatedOctahedron.csv",delimiter=",")
...
count=0
site_temp_record_total = []
atom_type_record_total = []
for count in range(4000001):
    event_index=np.random.choice([0,1,2],1).item()
    if event_index==0:
        flag,site_temp_record,atom_type_record=addition_event()
    elif event_index==1:
        flag,site_temp_record,atom_type_record=replacement_event()
    elif event_index==2:
        flag,site_temp_record,atom_type_record=deletion_event()

    if flag == True:
        site_temp_record_total.append(np.array([site_temp_record,event_index],dtype=object).flatten())
        atom_type_record_total.append(atom_type_record)
    ...
    if count%500000==0:
        np.save(path+"structure/"+"site_temp_record_total%d.npy"%count,site_temp_record_total)
        np.save(path+"structure/"+"atom_type_record_total%d.npy"%count,atom_type_record_total)
